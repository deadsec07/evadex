...
int_lines = [
    ax.plot([], [], [], lw=1, label=f"Interceptor {i+1}")[0] for i in range(NUM_INTERCEPTORS)
]
...
def update(frame):
    ...
    for ic in interceptors:
        if np.linalg.norm(missile.pos - ic.pos) < COLLISION_DIST:
            ax.text2D(
                0.5, 0.5, "GAME OVER", transform=ax.transAxes, color="red", fontsize=20, ha="center"
            )
    ...
    if np.linalg.norm(missile.pos - GOAL) < GOAL_DIST:
        ax.text2D(
            0.5, 0.5, "SUCCESS!", transform=ax.transAxes, color="green", fontsize=20, ha="center"
        )
        ani.event_source.stop()
